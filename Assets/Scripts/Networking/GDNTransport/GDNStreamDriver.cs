...
public class GDNStreamDriver {
    ...
    public Dictionary<int, GDNNetworkDriver.GDNConnection> gdnConnections = new Dictionary<int, GDNNetworkDriver.GDNConnection>();
    public Queue<GDNNetworkDriver.DriverTransportEvent> driverTransportEvents = new Queue<GDNNetworkDriver.DriverTransportEvent>();
    ...
    //private GDNNetworkDriver _gdnNetworkDriver;
    ...
    public void CreatServerInStream() {
        ...
        if (!serverInStreamExists) {
            ...
            _monobehaviour.StartCoroutine(MacrometaAPI.CreateStream(_gdnData, serverInStreamName, CreateServerInStreamCallback));
        }
    }
    ...
    public void CreatServerOutStream() {
        ...
        if (!serverOutStreamExists) {
            _gdnErrorHandler.isWaiting = true;
            _monobehaviour.StartCoroutine(MacrometaAPI.CreateStream(_gdnData, serverOutStreamName, CreateServerOutStreamCallback));
        }
    }
    ...
    public void CreatServerStatsStream() {
        ...
        if (!serverStatsStreamExists) {
            _gdnErrorHandler.isWaiting = true;
            _monobehaviour.StartCoroutine(MacrometaAPI.CreateStream(_gdnData, serverStatsStreamName, CreateServerStatsStreamCallback));
        }
    }
