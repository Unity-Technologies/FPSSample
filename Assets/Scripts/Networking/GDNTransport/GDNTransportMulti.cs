...
public class GDNTransportMulti :  INetworkTransport {
    ...
    public GDNNetworkDriverMulti gdnNetworkDriver;
    private GDNNetworkDriverMulti.GDNConnection[] m_IdToConnection;
    ...
    public GDNTransportMulti(bool isServer, int port = 0, int maxConnections = 16 )
    {
        
        GDNNetworkDriverMulti.overrideIsServer = true;
        GDNNetworkDriverMulti.overrideIsServerValue = isServer;
        GDNNetworkDriverMulti.isSocketPingOn = isSocketPingOn;
        GDNNetworkDriverMulti.isStatsOn = isStatsOn;
        GDNNetworkDriverMulti.sendDummyTraffic = sendDummyTraffic;
        gdnNetworkDriver= new GameObject().AddComponent<GDNNetworkDriverMulti>();

        MonoBehaviour.DontDestroyOnLoad(gdnNetworkDriver.gameObject);
        gdnNetworkDriver.port = port;

        m_IdToConnection = new GDNNetworkDriverMulti.GDNConnection[maxConnections];
    ...
    public bool NextEvent(ref TransportEvent e) {
        ...
        if (ev == GDNNetworkDriverMulti.DriverTransportEvent.Type.Empty)
            return false;
        e.data = new byte[8192];
        switch (ev) {
            case GDNNetworkDriverMulti.DriverTransportEvent.Type.Data:
                ...
                break;
            case GDNNetworkDriverMulti.DriverTransportEvent.Type.Connect:
                e.type = TransportEvent.Type.Connect;
                e.connectionId = driverTransportEvent.connectionId;
                break;
            case GDNNetworkDriverMulti.DriverTransportEvent.Type.Disconnect:
